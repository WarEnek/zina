...
COOKIEFORGE_CONTRACT_ADDRESS="${COOKIEFORGE_CONTRACT_ADDRESS:-${PROOFROLL_CONTRACT_ADDRESS:-}}"

for v in SEPOLIA_RPC_URL SUPABASE_URL SUPABASE_SERVICE_ROLE_KEY; do
  if [ -z "${!v:-}" ]; then
...
if [ -z "${COOKIEFORGE_CONTRACT_ADDRESS:-}" ]; then
  echo "Missing env var in function env: COOKIEFORGE_CONTRACT_ADDRESS (or legacy PROOFROLL_CONTRACT_ADDRESS)"
  exit 1
fi
...
bunx --bun supabase secrets set \
  ...
  COOKIEFORGE_CONTRACT_ADDRESS="$COOKIEFORGE_CONTRACT_ADDRESS" \
  PROOFROLL_CONTRACT_ADDRESS="$COOKIEFORGE_CONTRACT_ADDRESS" \
